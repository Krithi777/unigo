/**
 * DriverEmailVerifyWaiting — waits for driver to click their email link.
 * Polls backend every 4s. On success:
 *   - existing driver → setSession → RootNavigator routes to App
 *   - new driver      → navigate to DriverSetup
 */
...
import {
  checkDriverEmailVerified,
  resendDriverEmailVerification,
} from '../../services/authService';
import { useAuth } from '../../context/AuthContext';

type Nav = NativeStackNavigationProp<AuthStackParamList, 'DriverEmailVerifyWaiting'>;
type Route = RouteProp<AuthStackParamList, 'DriverEmailVerifyWaiting'>;

const POLL_INTERVAL_MS = 4000;

export default function DriverEmailVerifyWaiting() {
  ...
  const handleVerified = useCallback(
    (result: {
      ...
      if (!result.is_new_user && result.user) {
        ...
        navigation.navigate('DriverSetup', {
      ...
      <View style={styles.container}>
        ...
        <View style={styles.steps}>
          ...
          <View style={styles.step}>
            ...
            <Text style={styles.stepText}>Come back here — we'll continue your setup</Text>
          </View>
        </View>
